...
try {
  ...
  for (const lockName of ['SingletonLock', 'SingletonSocket', 'SingletonCookie']) {
    const lockPath = path.join(userDataPath, lockName);
...
function openSpotifyLoginWindow(owner, clientId) {
  return new Promise((resolve) => {
    const port = mainServerPort || 3000;
    ...
    const authUrl = `https://accounts.spotify.com/authorize?client_id=${encodeURIComponent(clientId)}&response_type=code&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(scopes)}`;
    ...
    loginWindow.webContents.on('did-redirect-navigation', (e, urlStr) => handleUrlChange(urlStr));
    loginWindow.webContents.on('did-start-navigation', (e, urlStr) => handleUrlChange(urlStr));

    loginWindow.on('closed', () => {
      if (!resolved) {
        resolved = true;
        resolve({ ok: false, error: 'USER_CANCELLED' });
      }
    });
  });
}
...
async function refreshPhantomSpotifyToken() {
  ...
  try {
    ...
    if (win.webContents.isLoading() || !url.includes('spotify.com')) {
