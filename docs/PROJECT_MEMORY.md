...
### 2026-06-27 - QQ/NE 全面移除，纯 Spotify 迁移

- 用户要求：移除所有 QQ/网易云/播客残留，只使用 Spotify 作为唯一音源供应商。
- 涉及文件：`public/index.html`、`desktop/main.js`。
- 关键修改：
  - `startupLoginStatusPromise` 一启动只调 `refreshSpotifyLoginStatus()` + `loadSpotifyCredentials()`，不再调 NE/QQ 状态检查。
  - `hasAnyPlatformLogin()` 改为 `hasPlatformLogin('spotify')`；`firstLoggedProvider()` 默认返回 `'spotify'`。
  - `platformMeta()`/`platformStatus()` 全部指向 Spotify。
  - `refreshSpotifyLoginStatus()` 使用 IPC `window.desktopWindow.getPhantomSpotifyToken()` 而非 HTTP API（因为 server.js 无 Spotify 路由）。
  - 搜索标签栏隐藏 NE/QQ/Podcast 按钮；`fetchMusicSearchResults()` 使用 Phantom Token 直接调用 Spotify Web API。
  - 用户模态框只显示 Spotify 账号信息。
  - 给 `phantom-spotify-get-token` IPC handler 添加 try/catch，防止未处理 Promise 拒绝。
  - 给 `startupLoginStatusPromise.then()` 添加 `.catch()`。
- 停止回退点：不要再把 `hasAnyPlatformLogin()` 改回检查 NE/QQ；不要再把 `startupLoginStatusPromise` 改回调 NE 状态；不要再把搜索路由改回 `/api/search`(NE)。
...
### 2026-06-27 - Spotify 全面修复：搜索、播放、登录检测、持久化

- 用户反馈问题：搜索不工作、播放不工作、登录后 app 不认、重启后登录丢失。
- 涉及文件：server.js、public/index.html、desktop/main.js
- 根本原因：
  - songProviderKey() 只返回 'qq' 或 'netease'，Spotify 歌曲被当作 NetEase 处理
  - playQueueAt() 中 Spotify 走 QQ/NE 回退路径（API 已删除）
  - 登录 token 仅存在内存中（phantomToken），未保存到磁盘
  - Electron phantom window 与 Puppeteer Chrome 使用不同浏览器 profile，登录状态不共享
- 修复内容：
  1. **server.js** 新增 Spotify OAuth：
     - 读取 .spotify-credentials（clientId/clientSecret）
     - 客户端凭证流（搜索无需登录）
     - 用户 token 刷新（使用 .spotify-token 的 refresh_token）
     - 路由：/api/spotify/token、/api/spotify/search、/api/spotify/me、/api/spotify/me/tracks、/api/spotify/save-token
  2. **public/index.html** 修复：
     - songProviderKey() 添加 'spotify' 识别
     - songSourceTagHtml() 显示 "SP" 标签
     - etchMusicSearchResults() 改用 /api/spotify/search
     - playQueueAt() 中 Spotify 歌曲调用 playPhantomSpotifyTrack(uri) 外部播放
     - 
efreshSpotifyLoginStatus() 改用 /api/spotify/token
     - loadHomeDiscover() 改用 /api/spotify/me/tracks
     - closeLoginModal() 自动刷新登录状态
  3. **desktop/main.js** 修复：
     - phantom-spotify-open-login 登录后等待 token、保存到 .spotify-token、初始化 Phantom Engine
     - token 保存时保留现有 refresh_token
- 登录后需注意：Puppeteer Chrome 与 Electron 使用不同 profile，首次播放会弹出 Chrome 窗口要求单独登录 Spotify；后续重启会自动保持登录。
- 停止回退点：不要恢复 QQ/NE 回退播放路径；不要把搜索改回 phantom window；不要删除 server.js 的 Spotify OAuth 路由。

### 2026-06-27 - 添加 server.js Spotify 后端路由（搜索/Token/用户/歌单）

- 用户反馈：搜索不工作、播放不工作——上次 commit 只改了前端，后端 server.js 完全没加 Spotify 路由。
- 根因：commit 279a9ed 在 public/index.html 中把搜索/播放全部改为调 /api/spotify/*，但 server.js 没有添加任何对应路由处理函数。所有 /api/spotify/* 请求落到静态文件处理器返回 Not Found（或错误 JSON 时前端显示空结果）。
- 涉及文件：server.js。
- 修复内容：
  1. **Spotify OAuth 模块**（文件头部，const updateDownloadJobs 之后插入）：
    - SPOTIFY_CREDENTIALS_PATH / SPOTIFY_TOKEN_PATH 常量
    - spotifyCredentials、spotifyServerToken、spotifyServerTokenExpiresAt、spotifyUserToken 状态变量
    - 启动时同步加载 .spotify-credentials 和 .spotify-token
    - getSpotifyClientToken() — 用 client credentials flow 获取匿名 token（用于搜索）
    - refreshSpotifyUserToken() — 用 refresh_token 刷新用户 token
    - getSpotifyApiToken(requireUser) — 统一获取 token：优先用户 token，过期则 refresh，失败则回退 client token
  2. **spotifyHttpsRequest() 辅助函数** — 用 Node.js https 模块替代全局 fetch（规避 Node v25.2.1 fetch 发送 Bearer header 时返回 Only valid bearer authentication supported 的 bug）
  3. **5 个路由**（天气路由之后、搜索区之前插入）：
    - GET /api/spotify/token — 返回 token + 登录态检测
    - GET /api/spotify/search?q=... — 搜索，返回 10 条结果
    - GET /api/spotify/me — 用户信息
    - GET /api/spotify/me/tracks — 已收藏歌曲
    - POST /api/spotify/save-token — 保存 OAuth token
- 验证结果：
  - /api/spotify/token -> {ok:true, token:BQD...}
  - /api/spotify/search?q=test -> {tracks:{items:[10条结果]}}
- 部署注意：改动在 C:\projects\Flux\server.js，需同步到运行版 E:\桌面\播放器软件\Mineradio\resources\app\server.js，重启 Mineradio.exe 生效。
- 停止回退点：不要删除 server.js 中的 Spotify OAuth 模块和路由；不要恢复 QQ/NE 搜索路由作为主要搜索源；不要删 spotifyHttpsRequest 换回 fetch 调 Spotify Web API。

### 2026-06-27 - Spotify ????????? Puppeteer/Chrome??? Electron ????

- ???????? Puppeteer ???? Chrome ????????? Chrome ???? app ??
- ???phantom-engine.js ?? `puppeteer-core` + `headless: false` ?? Chrome
- ???
  1. **desktop/phantom-engine.js** ?????
     - ?? puppeteer-core ??
     - ?? Electron BrowserWindow + `show: true` + off-screen ???x/y = -10000?
     - `skipTaskbar: true` + `frame: false` ??????????
     - ???? partition `persist:mineradio-spotify-login` ?? Spotify ???
     - `setBackgroundThrottling(false)` ??????????
     - ?????????20? ? 500ms???????
     - ?? `cleanup()` ????????
  2. **public/index.html**?Spotify ??????? `playing = true; setPlayIcon(true)`??? UI ????????????
  3. **desktop/main.js**??? handler ?? `phantomSpotifyEngine.cleanup()`
  4. **package.json**??? `puppeteer-core` ??
- ??????? `x: -10000, y: -10000, show: true, skipTaskbar: true, frame: false`
- ?????????? Puppeteer ???????? `show: true` ?? `show: false`???????
