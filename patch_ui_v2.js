...
const loginEnd = html.indexOf('</div>', html.indexOf('refresh-qr-btn', loginStart)) + 13;
...
if (html.length === oldLen) throw new Error("UI patch: login-modal replace failed");
oldLen = html.length;

// the search tabs might have \r\n instead of \n!
// use regex to match it reliably!
html = html.replace(/<div class="search-mode-toggles"[^>]*>[\s\S]*?<\/div>/, searchTabsNew);

fs.writeFileSync(target, html);
console.log('UI HTML patch VERIFIED successful!');
