...
html = html.replace(/async function loadHomeDiscover\(force\) \{[\s\S]*?renderHomeDiscover\(\);\s*\}\s*\}/, loadDiscoverNew);
if (html.length === oldLen) throw new Error("Daily patch: loadHomeDiscover replace failed");

oldLen = html.length;
html = html.replace(/async function playHomeDaily\(\) \{[\s\S]*?playQueueAt\(0\)\.catch\(function\(e\)\{\s*console\.warn\('\[HomeDailyPlay\]', e\);\s*\}\);\s*\}/, playHomeDailyNew);
if (html.length === oldLen) throw new Error("Daily patch: playHomeDaily replace failed");

fs.writeFileSync(target, html);
console.log('Daily patch VERIFIED successful!');
